...
Mit dem Befehl \tex{setuptabulate[split=yes]} können lange Tabellen auf mehrere Seiten verteilt werden.
Beim Umbruch verhalten sich die unterschiedlichen Linien etwas anders.
Normalerweise reicht die \tex{HL} Linie aus. 
Bei wiederholten Tabellenköpfen sollten jedoch \tex{FL} und \tex{LL} benutzt werden.

\starttabulate[|lB|l|]
\NC {\bf Linie} \NC {\bf Beschreibung} \NR
\NC \tex{HL}  \NC Horizontale Linie \NR\HL
\NC \tex{FL}  \NC Erste Linie \NR\FL
\NC \tex{ML}  \NC Mittlere Linie \NR\ML
\NC \tex{TL}  \NC Untere Linie \NR\TL
\NC \tex{TB}  \NC Leerzeile (optionale Höhe in \type{[]} angeben)  \NR\TB
\NC \tex{BL}  \NC Letzte Linie \NR\BL
\NC \tex{LL}  \NC Letzte Linie (Tabellenkopf)\NR\LL
\stoptabulate
...
Die \tex{setupTABLE} Befehle bestimmen das Aussehen der Tabelle.
...
Mit dem Befehl \tex{placetable} erhält man ein Gleitobjekt mit einer \tex{caption}, auf die man sich später beziehen kann.
