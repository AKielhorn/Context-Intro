...
Mit \type{\enablemode[]} wir eine Mode aktiviert, mit \type{\disablemode[]} wird er deaktiviert.
...
Modes können nicht geschachtelt werden, der \type{\startmode} gilt bis zum nächsten \type{\stopmode}-Befehl.
...
Zusätzlich zu den \type{\startmode}/\type{\stopmode}-Befehlen gibt es noch die invertierte Form \type{\startnotmode}/\type{\stopnotmode}.
Der so markierte Bereich wird ausgeführt, wenn der Mode {\em nicht} definiert ist.

Die Argumente der Befehle \type{\startmode} und \type{\startnotmode} verknüpfen die Argumente mit einer \type{oder}-Funktion,
ist eines der Argumente aktiv (bzw. inaktiv), wird er Inhalt des Modes ausgegeben.

Zusätzlich gibt es die Befehle \type{\startallmodes} und \type{\startnotallmodes}, hier müssen {\em alle} Bedingungen erfüllt sein,
...
Für kurze Texte gibt es außerdem noch vier \type{\doif}-Befehle
...
sowie drei \type{\doifelse}-Befehle
